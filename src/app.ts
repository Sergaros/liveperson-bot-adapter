...
if (process.argv.length > 2) {
  if (process.argv[2] == ARGUMENT_BOT_FRAMEWORK_ADAPTER_ONLY) {
    // No LivePerson
    console.log(
      `${ARGUMENT_BOT_FRAMEWORK_ADAPTER_ONLY} argument detected - will not use the LivePerson adapter`
    );
    useLivePersonAdapter = false;
  } else if (process.argv[2] == ARGUMENT_LIVEPERSON_ONLY) {
    // No Bot Framework adapter
    console.log(
      `${ARGUMENT_LIVEPERSON_ONLY} argument detected - will not use the Microsoft Bot Framework adapter`
    );
  ...
  server.listen(process.env.port || process.env.PORT || 3989, function() {
    console.log(
      `${server.name} listening to ${
        server.url
      } for events from the Bot Framework connector...`
    );
  });
...
if (useLivePersonAdapter) {
  ...
  livePersonBotAdapter
    .getListener()
    .on(LivePersonAgentListener.MESSAGE, async context => {
      (context.adapter as LivePersonBotAdapter).runMiddleware(
        context,
        async context => {
          await bot.onTurn(context);
        }
