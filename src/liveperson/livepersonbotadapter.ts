...
export class LivePersonBotAdapter extends BotAdapter {
  ...
  constructor(livePersonConfiguration: any) {
    ...
    try {
      ...
      console.error(
        `Failed to create/initialize the LivePerson agent: ${error}`
      );
    }
  ...
  public processActivity(
    ...
    throw new Error(
      "processActivity method is not implemented for LivePersonBotAdapter"
    );
  }
  ...
  protected initializeLivePersonAgent() {
    ...
    this.livePersonAgent.on("connected", msg => {
      ...
      console.log(
        "LivePerson agent connected:",
        this.livePersonAgent.conf.id || "",
        msg
      );
    ...
    this.livePersonAgent._reconnect = (delay = this.reconnectInterval, attempt = 1) => {
      this.livePersonAgent._retryConnection = setTimeout(()=>{
        this.livePersonAgent.reconnect();
          if (++attempt <= this.reconnectAttempts) { this.livePersonAgent._reconnect(delay * this.reconnectRatio, attempt) }
      }, delay * 1000)
    }
      ...
      notificationBody => {
        notificationBody.changes.forEach(change => {
          ...
          if (change.type === "UPSERT" && !openConvs[change.result.convId]) {
            ...
            const consumerId = change.result.conversationDetails.participants.filter(
              p => p.role === "CONSUMER"
            )[0].id;
    ...
    this.livePersonAgent.on("ms.MessagingEventNotification", body => {
      ...
      body.changes.forEach(c => {
        ...
        if (openConvs[c.dialogId]) {
          ...
          if (
            c.event.type === "AcceptStatusEvent" &&
            c.originatorId === this.livePersonAgent.agentId
          ) {
